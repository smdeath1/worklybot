...
def init_db():
    try:
        with sqlite3.connect("jobs.db", check_same_thread=False) as conn:
            ...
            if cur.fetchone()[0] == 0:
                cur.execute("INSERT INTO users (telegram_id, employer_code, subscription_active, subscription_start) VALUES (?, ?, ?, ?)",
                            (ADMIN_ID, "EMP_ADMIN", 1, datetime.now().strftime("%Y-%m-%d")))
                cur.execute("INSERT INTO vacancies (employer_code, city, description) VALUES (?, ?, ?)",
    ...
    try:
        ...
        with sqlite3.connect("jobs.db", check_same_thread=False) as conn:
            ...
            if not row:
                ...
                cur.execute("INSERT INTO users (telegram_id, employer_code, subscription_active) VALUES (?, ?, ?)",
                           (uid, employer_code, 0))
                conn.commit()
            else:
                employer_code = row[0]
